...
@section('content_header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Create Explain Question RIASEC</li>
    </ol>
@stop
...
@section('content')
    ...
    @endif
    <form class="form" method="POST" action="{{route('explainriasec.store')}}" enctype="multipart/form-data">
        {{ csrf_field() }}
                    ...
                    <select class="form-control" name="type">
                        @if(isset($typeRiasecs))
                            @foreach($typeRiasecs as $typeRiasec)
                                <option value="{{$typeRiasec->name}}">{{$typeRiasec->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
